function varargout = cosmologui_devel(varargin)
% COSMOLOGUI_DEVEL M-file for cosmologui_devel.fig
%      COSMOLOGUI_DEVEL, by itself, creates a new COSMOLOGUI_DEVEL or raises the existing
%      singleton*.
%
%      H = cosmologui_devel returns the handle to a new cosmologui_devel or the handle to
%      the existing singleton*.
%
%      cosmologui_devel('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in cosmologui_devel.M with the given input arguments.
%
%      cosmologui_devel('Property','Value',...) creates a new cosmologui_devel or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before cosmologui_devel_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to cosmologui_devel_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help cosmologui_devel
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @cosmologui_devel_OpeningFcn, ...
                   'gui_OutputFcn',  @cosmologui_devel_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% -----------------------------------------------------------------
% -----------------------------------------------------------------
% --- Executes just before cosmologui_devel is made visible.
function cosmologui_devel_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to cosmologui_devel (see VARARGIN)

% Choose default command line output for cosmologui_devel
...
fid=fopen('cosmologui_devel_prefs.mat');
if (fid>0)
    fclose(fid);
    htmp=load('cosmologui_devel_prefs');
...
% UIWAIT makes cosmologui_devel wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = cosmologui_devel_OutputFcn(hObject, eventdata, handles)
...
[handles.samples,handles.names,handles.isampled,handles.cuts,h,handles.colors]=...
    getsamps_devel('Position',[20 20 150 22]);
...
%save cosmologui_devel_prefs handles
if (isfield(handles,'xcol'))
  ...
  save cosmologui_devel_prefs xcol ycol zcol ...
...
% --- Executes on key press over cosmologui_devel with no controls selected.
function cosmologui_devel_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to cosmologui_devel (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
