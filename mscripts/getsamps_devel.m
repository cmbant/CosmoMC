function varargout = getsamps_devel(varargin)
% GETSAMPS_DEVEL M-file for getsamps_devel.fig
%      GETSAMPS_DEVEL, by itself, creates a new GETSAMPS_DEVEL or raises the existing
%      singleton*.
%
%      [SAMPLES, NAMES, ISAMPLED] = GETSAMPS_DEVEL returns the burn-removed samples info
%      SAMPLES, with column names NAMES as specified when running GETSAMPS_DEVEL
%
...
%
%      GETSAMPS_DEVEL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GETSAMPS_DEVEL.M with the given input arguments.
%
%      GETSAMPS_DEVEL('Property','Value',...) creates a new GETSAMPS_DEVEL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before getsamps_devel_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to getsamps_devel_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help getsamps_devel
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @getsamps_devel_OpeningFcn, ...
                   'gui_OutputFcn',  @getsamps_devel_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before getsamps_devel is made visible.
function getsamps_devel_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to getsamps_devel (see VARARGIN)
...
fid=fopen('getsamps_devel_prefs.mat');
if (fid>0)
    fclose(fid);
    htmp=load('getsamps_devel_prefs'); % sampsdir namesdir namesfile nburn nthin filescell
...
% UIWAIT makes getsamps_devel wait for user response (see UIRESUME)
% uiwait(handles.getsamps_devel);
uiwait; % don't let it assign the output arguments until we've got them


% --- Outputs from this function are returned to the command line.
function varargout = getsamps_devel_OutputFcn(hObject, eventdata, handles)
...
save getsamps_devel_prefs sampsdir namesdir namesfile nburn nthin samplesfiles usefiles

uiresume; % OK, can now assign the output arguments now we've finished


% --- Executes on key press over getsamps_devel with no controls selected.
function getsamps_devel_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to getsamps_devel (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% hmm, what if this window isn't called getsamps_devel ??
