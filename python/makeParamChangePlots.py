            ...
            if compares[0].result_marge.parWithName(param.name):
                ...
                if args.bands_sigma is not None and compi == 0:
                    ...
                    for i in range(len(args.bands_sigma)):
                        delta = args.bands_sigma[-i - 1] * array([sqrt(abs(sigma ** 2 - sigmas[-1] ** 2)) for sigma in sigmas])
                        c = fill_colors[i]
                        fill_between(lmaxs, means[-1] + delta, means[-1] - delta, facecolor=c, alpha=1, edgecolor=c, lw=0)
