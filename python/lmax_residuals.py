...
def plotFit(d, df=None, dref=None, plikLite=False, capt='', leg=True):
    ...
    plot(d[:, 0], (d[:, 1] - LCDM[:, 1]), '-b')
    if dref is not None:
        plot(dref[:, 0], (dref[:, 1] - LCDM[:, 1]), ':b')
    foreground = df is not None
    if foreground:
        plot(df[:, 0], (df[:, 1] - LCDMf[:, 1]), ':c')
        plot(df[:, 0], (df[:, 2] - LCDMf[:, 2]), '-r')
        plot(df[:, 0], (df[:, 3] - LCDMf[:, 3]), '-g')
        plot(df[:, 0], (df[:, 4] - LCDMf[:, 4]), '-m')
...
if True:
    ...
    if not separate:
        ...
        f, axs = subplots(4, 4, figsize=(34, 30))
        axs = axs.reshape(-1)
    else:
    ...
    def drawFor(i):
        ...
        d = loadtxt(root + '.theory_cl')[:2499, :]
        df = loadtxt(root + '.camspec_foregrounds')[:2499, :]
        bestfit = ResultObjs.bestFit(root)
        ...
        if len(extparam):
            ...
            basefit = ResultObjs.bestFit(baseroot)
        ...
        axvline(lmx, color='gray', ls=':')
        if separate: savefig(r'C:\tmp\Planck\final_Nov14\cuts\%s_Lmax%u.pdf' % (base, lmx), bbox_inches='tight')
    ...
    if separate:
        ...
        for i in range(len(lmaxs)):
            drawFor(i)
        savefig(r'C:\tmp\Planck\final_Nov14\cuts' + os.sep + base + '_Lmax_panels.pdf', bbox_inches='tight')
show()

sys.exit()


d = loadtxt(r'C:\Users\antony\Dropbox\Planck\final_Oct14\lensing\PostBorn\spectra_planck_uncorrected.data')
d2 = loadtxt(r'C:\Users\antony\Dropbox\Planck\final_Oct14\lensing\PostBorn\spectra_planck.data')
my = loadtxt(r'c:\work\dist\git\cosmomcplanck\camb\base_plikHM_TT_lowTEB_lensing_lenspotentialCls.dat')
...
for i in range(1, 6000):
    L = my[i, 0]
    var = var + (L + 0.5) * my[i, 5] / 4  # * 3.1415 * 2 / 4
    cum[i] = var
