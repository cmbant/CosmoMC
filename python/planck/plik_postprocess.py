...
#  del(lkl)
  ...
  for i in range(2, lmax + 1):
    print >> f, ("%6d" + " %14E"*16) % (i, Dl[i, 0, 0], Dl[i, 1, 1], Dl[i, 1, 2], Dl[i, 2, 2], Dl[i, 3, 3], Dl[i, 3, 4], Dl[i, 3, 5], Dl[i, 4, 4], Dl[i, 4, 5], Dl[i, 5, 5], Dl[i, 0, 3], Dl[i, 0, 4], Dl[i, 0, 5], Dl[i, 1, 4], Dl[i, 1, 5], Dl[i, 2, 5])
...
#  lkl = clik.clik(plikfile)
 ...
 # del(lkl)
  ...
  for chain in chains:
    ch = nm.loadtxt(chain)
    nmm = chain.split("_")
    if tag: nchain = "_".join(nmm[:-1] + [tag.replace('_', ''), nmm[-1]])
    else: nchain = chain
    f = open(nchain, "w")
    print nchain, ":",
