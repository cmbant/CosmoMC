...
if args.sigma_from_left is not None:
    compare_index = args.sigma_from_left
else:
    compare_index = -1
                    ...
                    if parse:
                        ...
                        if args.bands_sigma is not None and compi == 0:
                            ...
                            for i in range(len(args.bands_sigma)):
                                delta = args.bands_sigma[-i - 1] * array(
                                    [sqrt(abs(sigma ** 2 - sigmas_ref[compare_index] ** 2)) for sigma in sigmas])
                                c = fill_colors[i]
