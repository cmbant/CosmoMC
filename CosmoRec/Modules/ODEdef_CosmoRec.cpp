...
{   
    ...
    //==================================================================    
    ODE_HeI_effective::evaluate_effective_Rci_Ric_terms(z, Xe, XHeII*NH, HeI_Atoms, Ai_df_dx_HeI, Bi_df_dx_HeI, get_HeI_index, &LI.g[iHeI]);
    ODE_HeI_effective::evaluate_effective_Rij_terms(z, Tg, HeI_Atoms, RijVec_df_dx_HeI, get_HeI_index, &LI.g[iHeI]);    
    
    //==========================================================================
    // 2s two-photon channels (2S - 1S singlet channel)
    //==========================================================================
    ODE_effective::evaluate_2s_two_photon_decay(z, Tg, HeI_Atoms.Xi(0), HeI_Atoms.Xi(1), HeI_Atoms.Sing.Level(2, 0).Get_Dnu_1s2(), 
    ...
    //==================================================================
    if(flag_HI_absorption==1 && z<=zcrit_HI)
    {  
        ...
        evaluate_HI_abs_HeI(z, Hydrogen_Atoms.X(1, 0), NH, H_z, Tg, Te, 
                            HeI_Atoms, cosmos, 
                            H1s, DP_interpol_S, dXe_HeI_HI_abs_dt);
        ...
        if(z<=zcrit_HI_Int && flag_spin_forbidden==1)
        {
            ...
            evaluate_HI_abs_HeI_Intercombination(z, Hydrogen_Atoms.X(1, 0), NH, H_z, Tg, Te,
                                                 HeI_Atoms, cosmos, 
                                                 H1s, DP_interpol_T, dXe_HeI_HI_abs_dt);        
...
void fcn_effective(double z, Data_Level_III &LIII)
{
    ...
    //==================================================================
    update_Solver_vars_eff(&Level_I_temp.X[iHI], Hydrogen_Atoms, &Level_I_temp.X[Level_I_temp.index_HeI], HeI_Atoms, z, rho);
    ...
    // effective rates
    //==================================================================
    ODE_effective::evaluate_TM(z, Xe, parameters.fHe, rho, Tg, H_z, Level_I_temp.g[Level_I_temp.neq-1]);
    ODE_effective::evaluate_2s_two_photon_decay(z, Tg, Hydrogen_Atoms.X(0), Hydrogen_Atoms.X(1), Hydrogen_Atoms.Level(1).Get_Dnu_1s(), 
                                                const_HI_A2s_1s, Level_I_temp.g[iHI], Level_I_temp.g[iHI+1]);
    
    //==================================================================
    // HI Lyman-series
    //==================================================================
    int nLy_max=(int)min(Lyn_max_parameter, Hydrogen_Atoms.Get_n_of_Level(get_HI_index(get_number_of_resolved_levels()-1)));
    //
    for(int n=2; n<=nLy_max; n++)
    {
        Transition_Data_A T=Hydrogen_Atoms.Level(n, 1).Get_Data_A(1, 0);
        ODE_effective::evaluate_Ly_n_channel(z, Tg, Hydrogen_Atoms.X(0), Hydrogen_Atoms.X(n, 1), NH, H_z, T.A21, T.lambda21, T.Dnu, 
                                             Level_I_temp.g[iHI], Level_I_temp.g[iHI+Hydrogen_Atoms.Get_Level_index(n, 1)]);
    }   
    
    //==================================================================
    // these are the ones with effective rates that depend on Te
    //==================================================================    
    ODE_HI_effective::evaluate_effective_Rci_Ric_terms(z, Xe, Np, Hydrogen_Atoms, Ai_df_dx, Bi_df_dx, get_HI_index, &Level_I_temp.g[iHI]);
    ODE_HI_effective::evaluate_effective_Rij_terms(z, Tg, Hydrogen_Atoms, RijVec_df_dx, get_HI_index, &Level_I_temp.g[iHI]);    
    ...
    if(Diffusion_correction_is_on)
    {
        ...
        if(DI1_2s_correction) evaluate_HI_DI1_DI2_correction(z, Hydrogen_Atoms.X(0), Hydrogen_Atoms.X(1), 
                                                             Hydrogen_Atoms.Level(1).Get_Dnu_1s(), Tg, &Level_I_temp.g[iHI]);
    }
    
    //==================================================================
    // DM annihilation
    //==================================================================
    if(DM_annihilation) evaluate_DM_annihilation_terms(z, parameters.fHe, Tg, Xp, XHeII, fDM_CosmoRec, 
...
//==============================================================================
void fcn_for_jacobian_eval(int jX, int neq, double DXj, double z, double Tg, double NH, Data_Level_I &LI0, double *f)
{
    double Te=Tg;   
    ...
    // HI & HeI populations
    //------------------------------------------------------------------
    if(jX>=LI0.index_HI && jX<LI0.index_HeI) Hydrogen_Atoms.Set_population_of_level(jX-LI0.index_HI, LI0.X[jX]);
    else if(jX>=LI0.index_HeI && jX<LI0.neq-1) HeI_Atoms.Set_Xi(jX-LI0.index_HeI, LI0.X[jX]);       
    ...
    if(jX==LI0.neq-1)
    {
        ...
        update_Solver_vars_eff(&LI0.X[LI0.index_HI], Hydrogen_Atoms, &LI0.X[LI0.index_HeI], HeI_Atoms, z, LI0.X[LI0.neq-1], 0);
        //
        ODE_HI_effective::evaluate_effective_Rci_Ric_terms_for_Te_derivative(z, Xe, NH*Xp, Ai_df_dx, Bi_df_dx, get_HI_index, &f[LI0.index_HI]);
        ODE_HI_effective::evaluate_effective_Rij_terms(z, Tg, Hydrogen_Atoms, RijVec_df_dx, get_HI_index, &f[LI0.index_HI]);        
    }
    else 
    {
        //==========================================================================
        ...
        if(Diffusion_correction_is_on)
        {
            evaluate_HI_Diffusion_correction(z, Hydrogen_Atoms, NH, cosmos.H(z), Tg, &f[LI0.index_HI]);
            if(DI1_2s_correction) evaluate_HI_DI1_DI2_correction(z, Hydrogen_Atoms.X(0), Hydrogen_Atoms.X(1), 
                                                                 Hydrogen_Atoms.Level(1).Get_Dnu_1s(), Tg, &f[LI0.index_HI]);
        }      
            ...
            {  
                if(jX==LI0.index_HeI || jX==LI0.index_HeI+2 || jX==LI0.index_HI)
                {
                    ...
                    evaluate_HI_abs_HeI(z, Hydrogen_Atoms.X(1, 0), NH, cosmos.H(z), Tg, Te,
                                        HeI_Atoms, cosmos, H1s, DP_interpol_S, dXe_HeI_HI_abs_dt);
                ...
                // from intercombination line
                //==================================================================
                if(jX==LI0.index_HeI || jX==LI0.index_HeI+(int)HeI_Atoms.Get_Level_index(2, 1, 1, 1) || jX==LI0.index_HI)
                {
                    if(z<=zcrit_HI_Int && flag_spin_forbidden==1)
                    {
                        double dXe_HeI_HI_abs_dt=0.0;
                        evaluate_HI_abs_HeI_Intercombination(z, Hydrogen_Atoms.X(1, 0), NH, cosmos.H(z), Tg, Te, 
                                                             HeI_Atoms, cosmos, H1s, DP_interpol_T, dXe_HeI_HI_abs_dt);     
    ...
    //------------------------------------------------------------------
    if(jX>=LI0.index_HI && jX<LI0.index_HeI) Hydrogen_Atoms.Set_population_of_level(jX-LI0.index_HI, LI0.X[jX]);
    else if(flag_He==1 && jX>=LI0.index_HeI && jX<LI0.neq-1) HeI_Atoms.Set_Xi(jX-LI0.index_HeI, LI0.X[jX]);     
...
{ 
    ...
    //===========================================================================
    if(colX==neq-1) update_Solver_vars_eff(&LI0.X[LI0.index_HI], Hydrogen_Atoms, &LI0.X[LI0.index_HeI], HeI_Atoms, z, LI0.X[LI0.neq-1], 0);
    
    //===========================================================================
    // define numerical derivative
    //===========================================================================
    r[0]+=df_dx_2point_sym(Level_III_temp_JAC.g[0], Level_III_temp_JAC.y[0], DXj, eps);
    r[LI0.index_HI]+=df_dx_2point_sym(Level_III_temp_JAC.g[LI0.index_HI], Level_III_temp_JAC.y[LI0.index_HI], DXj, eps);
    //
    for(int m=0; m<neq_HI; m++){ int kl=LI0.index_HI+get_HI_index(m); r[kl]+=df_dx_2point_sym(Level_III_temp_JAC.g[kl], Level_III_temp_JAC.y[kl], DXj, eps); }
    if(flag_He==1)
    {
        r[LI0.index_HeI]+=df_dx_2point_sym(Level_III_temp_JAC.g[LI0.index_HeI], Level_III_temp_JAC.y[LI0.index_HeI], DXj, eps);
        for(int m=0; m<neq_HeI; m++){ int kl=LI0.index_HeI+get_HeI_index(m); r[kl]+=df_dx_2point_sym(Level_III_temp_JAC.g[kl], Level_III_temp_JAC.y[kl], DXj, eps); }
    }
...
void jac_effective(double z, Data_Level_III &LIII, int col)
{
    ...
    if(jac_effective_z!=z)
    {
        ...
        // copy Xi_H --> Atoms && update recombination coefficients
        //==================================================================
        update_Solver_vars_eff(&Level_I_temp.X[iHI], Hydrogen_Atoms, &Level_I_temp.X[iHeI], HeI_Atoms, z, Level_I_temp.X[Level_I_temp.neq-1]);
    }   
    ...
    if(col==0)
    {
        ODE_effective::df_drho_evaluate_TM(z, Xe, parameters.fHe, Tg, H_z, Level_I_temp.g[Level_I_temp.neq-1]);
        //
        compute_numerical_derivative(Level_I_temp.neq-1, neq, neq_HI, neq_HeI, z, Tg, NH, Level_I_temp, Level_I_temp.g);
        //
        for(int k=0; k<LIII.neq; k++) LIII.g[k]=Level_I_temp.g[k]*f0[Level_I_temp.neq-1]/f0[k];
    }
    //==================================================================
    // HI X1s derivatives
    //==================================================================
    ...
    {
        ODE_effective::df_dXHI1s_evaluate_TM(z, Xe, parameters.fHe, rho, Tg, H_z, Level_I_temp.g[Level_I_temp.neq-1]);
        //
        ODE_effective::df_dX1s_evaluate_2s_two_photon_decay(z, Tg, Hydrogen_Atoms.Level(1).Get_Dnu_1s(), const_HI_A2s_1s, Level_I_temp.g[iHI], Level_I_temp.g[iHI+1]); 
        //
        ODE_HI_effective::df_dXHI1s_evaluate_effective_Rci_Ric_terms(z, Xe*NH, Np, Ai_df_dx, Bi_df_dx, get_HI_index, &Level_I_temp.g[iHI]);
        if(flag_He==1) ODE_HeI_effective::df_dXHI1s_evaluate_effective_Rci_Ric_terms(z, XHeII*NH, Ai_df_dx_HeI, Bi_df_dx_HeI, get_HeI_index, &Level_I_temp.g[iHeI]);
        // 
        int nLy_max=(int)min(Lyn_max_parameter, Hydrogen_Atoms.Get_n_of_Level(get_HI_index(neq_HI-1)));
        for(int n=2; n<=nLy_max; n++)
        {
            Transition_Data_A T=Hydrogen_Atoms.Level(n, 1).Get_Data_A(1, 0);
            ODE_effective::df_dX1s_evaluate_Ly_n_channel(z, Tg, Hydrogen_Atoms.X(0), Hydrogen_Atoms.X(n, 1), NH, H_z, T.A21, T.lambda21, T.Dnu, 
                                                         Level_I_temp.g[iHI], Level_I_temp.g[iHI+Hydrogen_Atoms.Get_Level_index(n, 1)]);
        }
        //
        compute_numerical_derivative(iHI, neq, neq_HI, neq_HeI, z, Tg, NH, Level_I_temp, Level_I_temp.g);
        //
    ...
    else if(col>1 && col<2+neq_HI)
    {
        ...
        ///
        if(col==2) ODE_effective::df_dX2s_evaluate_2s_two_photon_decay(z, Tg, Hydrogen_Atoms.Level(1).Get_Dnu_1s(), const_HI_A2s_1s, Level_I_temp.g[iHI], Level_I_temp.g[iHI+1]);
        //
        ODE_HI_effective::df_dXi_evaluate_effective_Rci_Ric_terms(m, z, Ai_df_dx, Bi_df_dx, get_HI_index, &Level_I_temp.g[iHI]);
        //
        ODE_HI_effective::df_dXi_evaluate_effective_Rij_terms(m, z, Tg, Hydrogen_Atoms, RijVec_df_dx, get_HI_index, &Level_I_temp.g[iHI]);
        //
        ...
        if(n<=Lyn_max_parameter && l==1)
        {
            ...
            ODE_effective::df_dXnp_evaluate_Ly_n_channel(z, Tg, Hydrogen_Atoms.X(0), Hydrogen_Atoms.X(ik), NH, H_z, T.A21, 
                                                         T.lambda21, T.Dnu, Level_I_temp.g[iHI], Level_I_temp.g[iHI+ik]);
        }
        //
        compute_numerical_derivative(iHI+ik, neq, neq_HI, neq_HeI, z, Tg, NH, Level_I_temp, Level_I_temp.g);
        //
    ...
    {
        ODE_effective::df_dXHeI1s_evaluate_TM(z, Xe, parameters.fHe, rho, Tg, H_z, Level_I_temp.g[Level_I_temp.neq-1]);
        //
        ODE_effective::df_dX1s_evaluate_2s_two_photon_decay(z, Tg, HeI_Atoms.Sing.Level(2, 0).Get_Dnu_1s2(), const_HeI_A2s_1s, Level_I_temp.g[iHeI], Level_I_temp.g[iHeI+1]); 
        //
        ODE_HI_effective ::df_dXHeI1s_evaluate_effective_Rci_Ric_terms(z, Np, Ai_df_dx, Bi_df_dx, get_HI_index, &Level_I_temp.g[iHI]);
        ODE_HeI_effective::df_dXHeI1s_evaluate_effective_Rci_Ric_terms(z, Xe*NH, XHeII*NH, Ai_df_dx_HeI, Bi_df_dx_HeI, get_HeI_index, &Level_I_temp.g[iHeI]);
        //
        for(int k=1; k<neq_HeI; k++)
        {
            ...
            Transition_Data_HeI_A T=HeI_Atoms.Get_Trans_Data(ik, 1, 0, 0, 0);
            if(T.A21!=0) ODE_effective::df_dX1s_evaluate_Ly_n_channel(z, Tg, HeI_Atoms.Xi(0), HeI_Atoms.Xi(ik), 
                                                                      NH, H_z, T.A21, T.lambda21, T.Dnu, 
                                                                      Level_I_temp.g[iHeI], Level_I_temp.g[iHeI+ik], 
                                                                      HeI_Atoms.Get_gw(ik)/T.gwp);
        }
        //
        compute_numerical_derivative(iHeI, neq, neq_HI, neq_HeI, z, Tg, NH, Level_I_temp, Level_I_temp.g);
        //
    ...
    else if(flag_He==1 && col>2+neq_HI)
    {
        ...
        //
        if(3+neq_HI) ODE_effective::df_dX2s_evaluate_2s_two_photon_decay(z, Tg, HeI_Atoms.Sing.Level(2, 0).Get_Dnu_1s2(), const_HeI_A2s_1s, Level_I_temp.g[iHeI], Level_I_temp.g[iHeI+1]); 
        //
        ODE_HeI_effective::df_dXi_evaluate_effective_Rci_Ric_terms(m, z, Ai_df_dx_HeI, Bi_df_dx_HeI, get_HeI_index, &Level_I_temp.g[iHeI]);
        //
        ODE_HeI_effective::df_dXi_evaluate_effective_Rij_terms(m, z, Tg, HeI_Atoms, RijVec_df_dx_HeI, get_HeI_index, &Level_I_temp.g[iHeI]);        
        //
        ...
        if((l==1 && J==1) || (l==2 && J==2))
        {
            ...
            {
                Transition_Data_HeI_A T=HeI_Atoms.Get_Trans_Data(ik, 1, 0, 0, 0);
                ODE_effective::df_dXnp_evaluate_Ly_n_channel(z, Tg, HeI_Atoms.Xi(0), HeI_Atoms.Xi(ik), 
                                                             NH, H_z, T.A21, T.lambda21, T.Dnu, 
                                                             Level_I_temp.g[iHeI], Level_I_temp.g[iHeI+ik], 
                                                             HeI_Atoms.Get_gw(ik)/T.gwp);
            }
        ...
        //
        compute_numerical_derivative(iHeI+ik, neq, neq_HI, neq_HeI, z, Tg, NH, Level_I_temp, Level_I_temp.g);
        //
...
{    
    ...
    if(flag_He==1)
    {
        ...
        for(int k=0; k<nres_HeI; k++) f[nres_HI+k+3]=Level_III_temp.g[index_HeI+get_HeI_index(k)]/dz_dt;
    }
