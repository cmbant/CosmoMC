                      ...
                      Cosmos &cos, int wait=0)
{
    ...
    { 
        if(show_CosmoRec_mess>=0) 
            cout << "\n HeI 2P-1S intercombination line is switched off " << endl; 
    ...
    { 
        ...
        DI1_2s_correction=include_stimulated_term=include_absorption_term=0;
        ...
        { 
            ...
            DI1_2s_correction=include_stimulated_term=1;
...
void save_sol_for_PDE_solver(int iz, double z)
{
    if(!Diffusion_correction || (Diffusion_correction && iteration_count<Diff_iteration_max) )
    {
        ...
        for(int l=0; l<Level_I.nHIeq+1; l++) 
            pass_on_the_Solution_CosmoRec[iz][1+l]=Level_I.X[l];
