...
double pd_2P_Singlet_HeI(double Tg)
{
    ...
    return 1.0/(1.0+HeI_Atoms.phi_HeI_Lya_S_full_Gamma.Get_A21()/Bitot);
}
...
double pd_2P_Triplet_HeI(double Tg)
{
    ...
    return 1.0/(1.0+HeI_Atoms.phi_HeI_23p_1s2_full_Gamma.Get_A21()/Bitot);
}
...
double call_DP_Singlet(double T, double eta, double tauS, double pd)
{
    ...
    return DPesc_coh(tauS, eta, T, T, pd, HeI_Atoms.phi_HeI_Lya_S_full_Gamma, H1s);
}
...
double call_DP_Triplet(double T, double eta, double tauS, double pd)
{
    ...
    return DPesc_coh(tauS, eta, T, T, pd, HeI_Atoms.phi_HeI_23p_1s2_full_Gamma, H1s);
}
...
// tabulate DP
//===========================================================================================================
void create_Pesc_tables(double z_s, double z_e, 
                        Photoionization_Lyc &Lyc1s, 
                        Gas_of_HeI_Atoms &HeIA, Cosmos &cos)
{
    ...
    //
    double gw=3, gwp=1;
    double A21=HeIA.Get_Trans_Data(2, 1, 0, 0, 0).A21, lambda21=HeIA.Get_Trans_Data(2, 1, 0, 0, 0).lambda21;
    double sig_c=Lyc1s.sig_phot_ion_nuc();
    //
    ...
    for(int i=0; i<npz; i+=zstep)
    {
        ...
        for(int e=0; e<neta; e++)
        {
            ...
            for(int t=0; t<ntau_S; t++)
            {
                tau=exp(vtau[t]);
                //
//              DP=DPesc_coh(tau/pd, eta, Tg, Tg, pd, HeI_Atoms.phi_HeI_Lya_S_full_Gamma, H1s);
                DP=DPd_coh(tau, eta, Tg, Tg, HeI_Atoms.phi_HeI_Lya_S_full_Gamma, H1s);
                tB.DP.push_back(DP);
    ...
    for(int i=0, kl=0; i<npz; i+=zstep, kl++)
    {
        ...
        for(int e=0; e<neta; e++)
        {
            ...
            for(int t=0; t<ntau_T; t++)
            {
                tau=exp(vtau[t]);
                //
//              DP=DPesc_coh(tau, eta, Tg, Tg, 1.0, HeI_Atoms.phi_HeI_23p_1s2_full_Gamma, H1s);
                DP=DPd_coh(tau, eta, Tg, Tg, HeI_Atoms.phi_HeI_23p_1s2_full_Gamma, H1s);
                tB.DP.push_back(DP);
...
{    
    ...
    { 
        ...
        create_Pesc_tables(3450.0, 1500.0, Lyc1s, HeI_Atoms, cosmos);
