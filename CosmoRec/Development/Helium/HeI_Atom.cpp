...
{ 
    ...
    if(HeI_Atom_read_transition_data!=0) 
    {
        ...
        //===========================================================
        // Ly-a Singlet
        double A21=Sing.Level(2, 1).Get_A21(1, 0, 0, 0);
        double f=0.0; //Sing.Level(2, 1).Get_f(1, 0, 0, 0);
        double nu21=Sing.Level(2, 1).Get_nu21(1, 0, 0, 0);
        double lam21=Sing.Level(2, 1).Get_lambda21(1, 0, 0, 0);
        double Gamma=0;
        for(int m=0; m<(int)Sing.Level(2, 1).Get_n_down(); m++) Gamma+=Sing.Level(2, 1).Get_Trans_Data(m).A21;
//      cout << nu21 << " " << lam21 << " " << A21 << " " << Gamma << " " << f << endl;
        // initialize Voigt-profile for HeI 1s-2p 
        //phi_HeI_Lya_S.Set_atomic_data(5130498.338139691e+9, 5.843339929990197e-06, 1798900000.0, 0.27616, 4);
        phi_HeI_Lya_S.Set_atomic_data(nu21, lam21, A21, f, 4);
        phi_HeI_Lya_S_full_Gamma.Set_atomic_data_Gamma(nu21, lam21, A21, f, Gamma, 4);
        
        // Transition-Data for 3^3P_1
        A21=Trip.Level(2, 1, 1).Get_A21(1, 0, 0, 0);
        //f=Trip.Level(2, 1, 1).Get_f(1, 0, 0, 0);
        nu21=Trip.Level(2, 1, 1).Get_nu21(1, 0, 0, 0);
        lam21=Trip.Level(2, 1, 1).Get_lambda21(1, 0, 0, 0);
        Gamma=0;
        for(int m=0; m<(int)Trip.Level(2, 1, 1).Get_n_down(); m++) Gamma+=Trip.Level(2, 1, 1).Get_Trans_Data(m).A21;
//      cout << nu21 << " " << lam21 << " " << A21 << " " << Gamma << " " << f << endl;
        // initialize Voigt-profile for HeI 1s-2p Triplet 
        phi_HeI_23p_1s2.Set_atomic_data(nu21, lam21, A21, f, 4);
        //phi_HeI_23p_1s2_mod.Set_atomic_data_Gamma(5069099.184149439e+9, 5.91411702768454e-06, 177.58, 2.7935e-08, 1.0216e+07+177.58, 4);
        phi_HeI_23p_1s2_full_Gamma.Set_atomic_data_Gamma(nu21, lam21, A21, f, Gamma, 4);
        
        // Transition-Data for n^1 P_1 - 1^1 S_0
        for(int k=3; k<=min(Get_nShells(), 10); k++)
        {
            A21=Sing.Level(k, 1).Get_A21(1, 0, 0, 0);
            //f=Sing.Level(k, 1).Get_f(1, 0, 0, 0);
            nu21=Sing.Level(k, 1).Get_nu21(1, 0, 0, 0);
            lam21=Sing.Level(k, 1).Get_lambda21(1, 0, 0, 0);
            Gamma=0;
            for(int m=0; m<(int)Sing.Level(k, 1).Get_n_down(); m++) Gamma+=Sing.Level(k, 1).Get_Trans_Data(m).A21;
            
            if(mflag>=1) cout << " Initializing profile for " << k << "^1 P_1 - 1^1 S_0 transition:" 
                              << nu21 << " " << lam21 << " " << A21 << " " << Gamma << " " << f << endl;
            
            // initialize Voigt-profiles for HeI 1s-np series
            phi_HeI_Lyn_S[k].Set_atomic_data_Gamma(nu21, lam21, A21, f, Gamma, 4);
        }
    }
