...
//
//==================================================================================
double HI_Get_z(int iz, vector<vector<double> > &Xi_Data)
{ return Xi_Data[iz][0]; }

double HI_Get_Xi(int iz, vector<vector<double> > &Xi_Data, int i)
{ return Xi_Data[iz][i]; }
                           ...
                           vector<vector<double> > &X_Data)
{
    ...
    if(Load_Populations_HI_spline_memory_was_allocated)
        if(HI_Xi_spline[0].nz!=nz || 
           HI_Xi_spline.size()!=X_Data[0].size()-3) clear_Xi_HI();
    
    
    //==============================================================================
    // check if redshift ranges is withing bounds
    //==============================================================================
    if(zs/1.0001>HI_Get_z(0, X_Data) || ze*1.0001<HI_Get_z(nz-1, X_Data))
    { 
