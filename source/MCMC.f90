    ...
    if (accpt) then
        if (num_accept> burn_in .and. checkpoint_burn ==0) then
    ...
    do while (num <= samples_to_get)
        ...
        if (indep_sample /= 0) then
            if (num_accept>= burn_in .and. mod(num, indep_sample)==0 .and. checkpoint_burn==0) then
        ...
        if (accpt) then
            if (num_accept> burn_in .and. CurLike/=LogZero ) then
                ...
                if (rmult /= 0 .and. checkpoint_burn==0) then
                    output_lines = output_lines +1
                    call WriteParams(CurParams,rmult,CurLike)
                elseif (rmult /= 0 .and. checkpoint_burn/=0) then
                    checkpoint_burn = checkpoint_burn-1
                end if
            end if
